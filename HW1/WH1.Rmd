...
## Generate Distributions and Sample Means
...
distributions <- 
  tribble(
    ...
    "rnorm", list(n = 1000),
    "rexp", list(n = 1000),
    "rbinom", list( n = 1000, size = 1, p = 0.1)
    )
...
distributions %>% 
  mutate(samp3 = map(.$distr, sampling, 3),
         samp10 = map(.$distr, sampling, 10),
         samp100 = map(.$distr, sampling, 100)
  ) 
...
df %>% 
  ...
  ggtitle('Central Limit Theorem', 
    subtitle = 'The distributions of the means seem to always be bell shape')
...
df %>% 
  ...
  facet_grid(.~n, scales = "free") +
  ggtitle('Central Limit Theorem', 
    subtitle = 'The distributions of the means are bell shaped when sample is > 30')
